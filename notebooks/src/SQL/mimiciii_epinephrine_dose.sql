...
select
    stay_id
    , endtime
    -- , (endtime - (LAG(endtime, 1) OVER (partition by icustay_id, vaso order by endtime))) AS delta
    ...
    , case
        ...
        --when (endtime - (LAG(endtime, 1) OVER (partition by icustay_id, vaso order by endtime))) > (interval '4 hours') then 1
      else null
      end as vaso_start
...
-- select
--   icustay_id, endtime, vaso, vaso_rate, vaso_amount
--     , vaso_stopped
--     , vaso_start
--     , vaso_first
--     , vaso_stop
-- from vasocv6 order by icustay_id, endtime;
